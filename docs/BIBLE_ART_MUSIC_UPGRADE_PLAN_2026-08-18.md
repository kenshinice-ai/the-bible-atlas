...
- 本轮落地：41 人物徽章 + 13 时代徽章、25 张多雷版画、59 条经文引用、38 条逐字核验引文、13 条音乐接受链接、1 张确定性总览母图
...
| A2 | 公版艺术描绘分批扩容（多雷版画为主） | 每批 S | 本轮第一批 |
...
**A1 符号库**（首批 40 位）遵循传统象征物，每条都在 `symbol_meaning` 里写明依据经文或传统出处；不发明新符号。
...
实测结果（2026-08-18）：该规则在生成期抓出 14 处真实转写差异——弯引号、和合本异体字「裏/着/作」、编者夹注「除去〔或作：背負〕」、以及 WEB 把约翰福音 20:18 写成间接引语。订正后 38 条引文 × 2 语言全部通过，`editorially_entered` 在本批次中没有留下任何条目。
