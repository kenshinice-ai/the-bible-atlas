...
- **新约多雷批次已于同日补齐并发布**（commit `43a10c2`，deployment `f323ba32`）：Wikimedia 限流窗口约半小时后自行解除，把单张间隔放宽到 15 秒后 14 张一次跑完。圣经媒体总数由 28 升至 **42**（人物形象 25 / 事件场景 16 / 地点影像 1），媒体目录 43 个文件 checksum 全对。
- 线上核对（绕过边缘缓存）：39 张多雷版画全部 `illustrative` 且带文件页与许可 URL、逐张 HTTP 200；耶稣徽章仍为 `chi-rho`、其条目挂 1 张版画且说明同时引述尼西亚二会与海德堡/韦斯敏斯特两种传统；受难系列 0 张。**注意**：Cloudflare 边缘会缓存 `data/*.json`，部署后立即读取可能拿到旧副本，核对时须加 query 串绕过。
...
本阶段已通过：`npm run typecheck`、`npm test`（API 5 + Web 31）、`npm run build`、`npm run verify:postgis`；PostgreSQL 18 + PostGIS 全量 migration/seed bootstrap 及重复幂等执行；48/48 映射、96 事件、43 地点关联、48 来源关联、36 条双语关系翻译 SQL 审计；API atlas/detail/search（搜索同一艺术家只返回一个人物结果）；最终静态包浏览器验收（桌面/390px、中英切换保留人物与关系状态、旧 `artist:` 深链接归一、人物→关系图 48 节点/18 连线、控制台无 error/warn）通过。独立站已发布 production：**https://european-art-history-atlas.pages.dev**，本次 deployment `d3b92eb0`（预览地址 `https://d3b92eb0.european-art-history-atlas.pages.dev`）；线上 JSON 返回 48 人物 / 48 艺术家 / 96 作品 / 96 事件 / 18 关系，HTTP 200。源码实现提交为 `5229286d4617dc38871ec86f25d91ad6786ee86e`，最终 handoff 提交为 `d0417ea`；静态构建关键哈希：`index.html` `760919b4…a989c`、`index-qED-wdp5.js` `7b996bac…86633`、`index-D_ZGmncc.css` `40398db4…129a15`。源码包：[European-Art-History-Atlas-R7-20260802-source.zip](../release/European-Art-History-Atlas-R7-20260802-source.zip)，SHA-256：`12daad4d94ea6d5229d8223c2cce3c71fd47abef9d43b7d5460b958f6288765b`。详见 [`docs/HANDOFF_DECISIONS_2026-08-02.md`](HANDOFF_DECISIONS_2026-08-02.md)。
...
验证门禁：`npm run typecheck`、`npm test`、`npm run build`、`npm run verify:postgis`、migration+seed bootstrap（含重复幂等执行）、计数/双语/时间/孤儿审计、API atlas/search/detail、profile 静态构建，以及桌面/中英切换/时代筛选/实体深链接/390px 响应式浏览器验收均通过。`verify:postgis` 已同步当前 001–009 migration 与 API `4.0.0` health contract，同时保留 001→002 升级夹具。
