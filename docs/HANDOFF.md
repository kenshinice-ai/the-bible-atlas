...
本阶段已通过：`npm run typecheck`、`npm test`（API 5 + Web 31）、`npm run build`、`npm run verify:postgis`；PostgreSQL 18 + PostGIS 全量 migration/seed bootstrap 及重复幂等执行；48/48 映射、96 事件、43 地点关联、48 来源关联、36 条双语关系翻译 SQL 审计；API atlas/detail/search（搜索同一艺术家只返回一个人物结果）；最终静态包浏览器验收（桌面/390px、中英切换保留人物与关系状态、旧 `artist:` 深链接归一、人物→关系图 48 节点/18 连线、控制台无 error/warn）通过。独立站已发布 production：**https://european-art-history-atlas.pages.dev**，本次 deployment `d3b92eb0`（预览地址 `https://d3b92eb0.european-art-history-atlas.pages.dev`）；线上 JSON 返回 48 人物 / 48 艺术家 / 96 作品 / 96 事件 / 18 关系，HTTP 200。源码实现提交为 `5229286d4617dc38871ec86f25d91ad6786ee86e`；静态构建关键哈希：`index.html` `760919b4…a989c`、`index-qED-wdp5.js` `7b996bac…86633`、`index-D_ZGmncc.css` `40398db4…129a15`。详见 [`docs/HANDOFF_DECISIONS_2026-08-02.md`](HANDOFF_DECISIONS_2026-08-02.md)。
