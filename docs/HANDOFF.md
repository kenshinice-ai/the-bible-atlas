...
0. **重大方向调整(2026-07-27,用户决定)**:项目**只服务于圣经**,进行品牌重塑(改用更庄重的圣经主题名称)+ 神圣内容融入(和合本/KJV 公有领域经文题词、典雅界面文案)+ UI/UX 完善
   - 已新增可复用代理定义 `.claude/agents/liturgical-design-director.md`(神职人员 × UI/UX 设计总监双重人格,用于圣经内容呈现与神圣氛围设计的评审)
   - 评审代理正在产出实施方案 `docs/design/sacred-rebrand-plan.md`(命名、13 时代经文题词、文案圣化清单、靛蓝+圣金令牌表、Bible-only 化、P0-P2 优先级)
