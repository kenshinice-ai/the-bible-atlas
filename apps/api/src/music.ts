...
export async function loadMusicAtlas(db: Database, workId: string, requestedLocale: string, fallbackLocale: string) {
  ...
  const [musicPeople, compositions, musicStyles, instruments, musicInstitutions, scoreFragments] = await Promise.all([
    db.query(`SELECT c.slug AS "characterSlug",p.primary_role AS "primaryRole",ch.slug AS "chapterSlug",
      COALESCE((SELECT json_agg(r.role ORDER BY r.role) FROM music_person_roles r WHERE r.character_id=p.character_id),'[]'::json) AS "roleCodes",
      COALESCE((SELECT json_agg(DISTINCT co.slug) FROM composition_contributors cc JOIN compositions co ON co.id=cc.composition_id WHERE cc.character_id=p.character_id),'[]'::json) AS "compositionSlugs",
      COALESCE((SELECT json_agg(DISTINCT ms.slug) FROM music_person_styles mps JOIN music_styles ms ON ms.id=mps.style_id WHERE mps.character_id=p.character_id),'[]'::json) AS "styleSlugs",
      COALESCE((SELECT json_agg(DISTINCT i.slug) FROM music_person_instruments mpi JOIN instruments i ON i.id=mpi.instrument_id WHERE mpi.character_id=p.character_id),'[]'::json) AS "instrumentSlugs",
      COALESCE((SELECT json_agg(DISTINCT mi.slug) FROM music_person_institutions mpi JOIN music_institutions mi ON mi.id=mpi.institution_id WHERE mpi.character_id=p.character_id),'[]'::json) AS "institutionSlugs",
      COALESCE((SELECT json_agg(DISTINCT e.slug ORDER BY e.slug) FROM music_person_event_links mpe JOIN events e ON e.id=mpe.event_id WHERE mpe.character_id=p.character_id),'[]'::json) AS "eventSlugs",
      COALESCE((SELECT json_agg(${sourceTitle} ORDER BY ${sourceTitle}) FROM character_sources cs JOIN sources s ON s.id=cs.source_id ${sourceJoin("s")} WHERE cs.character_id=p.character_id),'[]'::json) AS "sourceTitles"
    ...
    db.query(`SELECT co.id,co.slug,pc.slug AS "primaryComposerSlug",ch.slug AS "chapterSlug",
      ...
      co.confidence,co.catalogue_number AS "catalogueNumber",co.genre,co.form,co.key_signature AS "keySignature",co.work_status AS "workStatus",
      ...
      COALESCE((SELECT json_agg(DISTINCT c.slug) FROM composition_contributors cc JOIN characters c ON c.id=cc.character_id WHERE cc.composition_id=co.id),'[]'::json) AS "contributorSlugs",
      COALESCE((SELECT json_agg(DISTINCT s.slug) FROM composition_styles cs JOIN music_styles s ON s.id=cs.style_id WHERE cs.composition_id=co.id),'[]'::json) AS "styleSlugs",
      COALESCE((SELECT json_agg(DISTINCT i.slug) FROM composition_instruments ci JOIN instruments i ON i.id=ci.instrument_id WHERE ci.composition_id=co.id),'[]'::json) AS "instrumentSlugs",
      COALESCE((SELECT json_agg(DISTINCT i.slug) FROM composition_institutions ci JOIN music_institutions i ON i.id=ci.institution_id WHERE ci.composition_id=co.id),'[]'::json) AS "institutionSlugs",
      COALESCE((SELECT json_agg(DISTINCT e.slug ORDER BY e.slug) FROM composition_event_links ce JOIN events e ON e.id=ce.event_id WHERE ce.composition_id=co.id),'[]'::json) AS "eventSlugs",
      COALESCE((SELECT json_agg(sf.slug ORDER BY sf.sort_order) FROM score_fragments sf WHERE sf.composition_id=co.id),'[]'::json) AS "scoreFragmentSlugs",
      COALESCE((SELECT json_agg(${sourceTitle} ORDER BY ${sourceTitle}) FROM composition_sources cs JOIN sources s ON s.id=cs.source_id ${sourceJoin("s")} WHERE cs.composition_id=co.id),'[]'::json) AS "sourceTitles"
    ...
    db.query(`SELECT ms.id,ms.slug,ms.style_kind AS "styleKind",ch.slug AS "chapterSlug",ms.start_year AS "startYear",ms.end_year AS "endYear",
      ...
      COALESCE((SELECT json_agg(DISTINCT c.slug) FROM music_person_styles ps JOIN characters c ON c.id=ps.character_id WHERE ps.style_id=ms.id),'[]'::json) AS "characterSlugs",
      COALESCE((SELECT json_agg(DISTINCT co.slug) FROM composition_styles cs JOIN compositions co ON co.id=cs.composition_id WHERE cs.style_id=ms.id),'[]'::json) AS "compositionSlugs",
      COALESCE((SELECT json_agg(${sourceTitle} ORDER BY ${sourceTitle}) FROM music_style_sources mss JOIN sources s ON s.id=mss.source_id ${sourceJoin("s")} WHERE mss.style_id=ms.id),'[]'::json) AS "sourceTitles"
    ...
    db.query(`SELECT i.id,i.slug,i.family,i.hornbostel_sachs_code AS "hornbostelSachsCode",i.mimo_term AS "mimoTerm",
      ...
      CASE WHEN t.name IS NULL THEN $3::locale_code ELSE $2::locale_code END AS "resolvedLocale",(t.name IS NULL) AS "fallbackUsed",COALESCE(t.status,f.status) AS "translationStatus",
      COALESCE((SELECT json_agg(DISTINCT c.slug) FROM music_person_instruments pi JOIN characters c ON c.id=pi.character_id WHERE pi.instrument_id=i.id),'[]'::json) AS "characterSlugs",
      COALESCE((SELECT json_agg(DISTINCT co.slug) FROM composition_instruments ci JOIN compositions co ON co.id=ci.composition_id WHERE ci.instrument_id=i.id),'[]'::json) AS "compositionSlugs",
      COALESCE((SELECT json_agg(${sourceTitle} ORDER BY ${sourceTitle}) FROM instrument_sources ins JOIN sources s ON s.id=ins.source_id ${sourceJoin("s")} WHERE ins.instrument_id=i.id),'[]'::json) AS "sourceTitles"
    ...
    db.query(`SELECT i.id,i.slug,l.slug AS "locationSlug",i.institution_type AS "institutionType",i.founded_year AS "foundedYear",i.closed_year AS "closedYear",
      COALESCE(t.name,f.name) name,COALESCE(t.summary,f.summary) summary,
      CASE WHEN t.name IS NULL THEN $3::locale_code ELSE $2::locale_code END AS "resolvedLocale",(t.name IS NULL) AS "fallbackUsed",COALESCE(t.status,f.status) AS "translationStatus",
      COALESCE((SELECT json_agg(DISTINCT c.slug) FROM music_person_institutions pi JOIN characters c ON c.id=pi.character_id WHERE pi.institution_id=i.id),'[]'::json) AS "characterSlugs",
      COALESCE((SELECT json_agg(DISTINCT co.slug) FROM composition_institutions ci JOIN compositions co ON co.id=ci.composition_id WHERE ci.institution_id=i.id),'[]'::json) AS "compositionSlugs",
      COALESCE((SELECT json_agg(${sourceTitle} ORDER BY ${sourceTitle}) FROM music_institution_sources mis JOIN sources s ON s.id=mis.source_id ${sourceJoin("s")} WHERE mis.institution_id=i.id),'[]'::json) AS "sourceTitles"
    ...
    db.query(`SELECT frag.id,frag.slug,co.slug AS "compositionSlug",frag.start_measure AS "startMeasure",frag.end_measure AS "endMeasure",
      frag.notation_kind AS "notationKind",frag.svg_asset_path AS "svgAssetPath",frag.timing_asset_path AS "timingAssetPath",
