...
export function createApp(db: Database) {
  ...
  app.get("/api/works/:slug/atlas", async (request, response, next) => {
    try {
      ...
      const workResult = await db.query(`SELECT w.id,w.slug,w.author_name AS "authorName",w.content_mode AS "contentMode",w.map_layer AS "mapLayer",w.default_locale,
        COALESCE(t.title,f.title) title,COALESCE(t.summary,f.summary) summary,CASE WHEN t.title IS NULL THEN w.default_locale ELSE $2::locale_code END AS "resolvedLocale",(t.title IS NULL) AS "fallbackUsed"
      ...
      const [characters, locations, events, routes, relations, sources] = await Promise.all([
        db.query(`SELECT c.slug,COALESCE(t.name,f.name) name,COALESCE(t.summary,f.summary) summary FROM characters c LEFT JOIN character_translations t ON t.character_id=c.id AND t.locale=$2 AND t.status='published' LEFT JOIN character_translations f ON f.character_id=c.id AND f.locale=$3 AND f.status='published' WHERE c.work_id=$1 ORDER BY c.sort_order`, [workId,requestedLocale,work.default_locale]),
        db.query(`SELECT l.slug,l.layer,ST_X(l.geom::geometry) lng,ST_Y(l.geom::geometry) lat,l.canvas_x::float "canvasX",l.canvas_y::float "canvasY",COALESCE(t.name,f.name) name,COALESCE(t.summary,f.summary) summary FROM locations l LEFT JOIN location_translations t ON t.location_id=l.id AND t.locale=$2 AND t.status='published' LEFT JOIN location_translations f ON f.location_id=l.id AND f.locale=$3 AND f.status='published' WHERE l.work_id=$1 ORDER BY l.sort_order`, [workId,requestedLocale,work.default_locale]),
        db.query(`SELECT e.slug,e.start_date AS "startDate",e.end_date AS "endDate",e.sequence,e.reality,COALESCE(t.title,f.title) title,COALESCE(t.summary,f.summary) summary,COALESCE(json_agg(DISTINCT el.location_id) FILTER(WHERE el.location_id IS NOT NULL),'[]') "locationIds" FROM events e LEFT JOIN event_translations t ON t.event_id=e.id AND t.locale=$2 AND t.status='published' LEFT JOIN event_translations f ON f.event_id=e.id AND f.locale=$3 AND f.status='published' LEFT JOIN event_locations el ON el.event_id=e.id WHERE e.work_id=$1 GROUP BY e.id,t.title,t.summary,f.title,f.summary ORDER BY e.sequence`, [workId,requestedLocale,work.default_locale]),
        db.query(`SELECT r.id,r.slug,r.layer,r.certainty,COALESCE(t.name,f.name) name,COALESCE(t.summary,f.summary) summary,COALESCE(json_agg(json_build_object('position',rw.position,'locationSlug',l.slug) ORDER BY rw.position) FILTER(WHERE rw.location_id IS NOT NULL),'[]') waypoints FROM routes r LEFT JOIN route_translations t ON t.route_id=r.id AND t.locale=$2 AND t.status='published' LEFT JOIN route_translations f ON f.route_id=r.id AND f.locale=$3 AND f.status='published' LEFT JOIN route_waypoints rw ON rw.route_id=r.id LEFT JOIN locations l ON l.id=rw.location_id WHERE r.work_id=$1 GROUP BY r.id,t.name,t.summary,f.name,f.summary ORDER BY r.sort_order`, [workId,requestedLocale,work.default_locale]),
        db.query(`SELECT fc.slug "fromSlug",tc.slug "toSlug",r.relation_type "relationType",COALESCE(t.label,f.label) label FROM character_relations r JOIN characters fc ON fc.id=r.from_character_id JOIN characters tc ON tc.id=r.to_character_id LEFT JOIN relation_translations t ON t.relation_id=r.id AND t.locale=$2 AND t.status='published' LEFT JOIN relation_translations f ON f.relation_id=r.id AND f.locale=$3 AND f.status='published' WHERE r.work_id=$1`, [workId,requestedLocale,work.default_locale]),
  ...
  app.get("/api/search", async (request, response, next) => {
    try {
      ...
      const result = await db.query(`SELECT kind,slug,label,work_slug AS "workSlug" FROM (
