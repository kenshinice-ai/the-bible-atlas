    ...
    try {
      ...
      if (kind === "relationship") {
        ...
        const queries: Record<string, string> = {
          ...
          artist: `SELECT COALESCE(t.modern_status,f.modern_status,'') AS "modernStatus",COALESCE(array_to_string(t.period_titles,' · '),array_to_string(f.period_titles,' · '),'') AS "periodTitles" FROM artists a LEFT JOIN artist_translations t ON t.artist_id=a.id AND t.locale=$2 AND t.status='published' LEFT JOIN artist_translations f ON f.artist_id=a.id AND f.locale=$3 AND f.status='published' WHERE a.work_id=$1 AND a.slug=$4`,
  ...
  app.get("/api/search", async (request, response, next) => {
    try {
      ...
      const result = await db.query(`SELECT kind,slug,label,context,work_slug AS "workSlug",$1::locale_code AS "resolvedLocale",false AS "fallbackUsed",'published'::translation_status AS "translationStatus" FROM (
        ...
        UNION ALL SELECT 'artist',a.slug,at.name,at.summary,w.slug FROM artist_translations at JOIN artists a ON a.id=at.artist_id JOIN works w ON w.id=a.work_id WHERE at.locale=$1 AND at.status='published' AND (at.name||' '||at.summary||' '||at.modern_status||' '||array_to_string(at.period_titles,' ')) ILIKE '%'||$2||'%'
        UNION ALL SELECT 'artwork',aw.slug,wt.title,wt.summary,w.slug FROM artwork_translations wt JOIN artworks aw ON aw.id=wt.artwork_id JOIN works w ON w.id=aw.work_id WHERE wt.locale=$1 AND wt.status='published' AND (wt.title||' '||wt.summary||' '||aw.medium) ILIKE '%'||$2||'%'
