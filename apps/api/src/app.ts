...
const EntityKindSchema = z.enum(["character", "event", "location", "route", "relationship"]);
    ...
    try {
      ...
      const [characters, locations, events, routes, relations, sources, chronologies, media, chapters, groups] = await Promise.all([
        ...
        db.query(`SELECT ma.id,ml.entity_kind AS "entityKind",ml.entity_id AS "entityId",ma.asset_source AS "assetSource",ma.asset_licence AS "assetLicence",ma.asset_author AS "assetAuthor",ma.asset_url AS "assetUrl",ma.attribution_text AS "attributionText",CASE WHEN $2='zh-CN' THEN ma.alt_text_zh ELSE ma.alt_text_en END AS "altText"
          FROM media_assets ma JOIN media_links ml ON ml.media_id=ma.id WHERE ml.entity_id IN (SELECT id FROM characters WHERE work_id=$1 UNION SELECT id FROM events WHERE work_id=$1 UNION SELECT id FROM locations WHERE work_id=$1 UNION SELECT id FROM routes WHERE work_id=$1 UNION SELECT id FROM character_relations WHERE work_id=$1) ORDER BY ml.sort_order`, [workId, requestedLocale]),
      ...
      response.json({
        ...
        chapters: chapters.rows, groups: groups.rows,
      });
    } catch (error) { next(error); }
  });
  ...
  app.get("/api/search", async (request, response, next) => {
    try {
      ...
      const result = await db.query(`SELECT kind,slug,label,context,work_slug AS "workSlug",$1::locale_code AS "resolvedLocale",false AS "fallbackUsed",'published'::translation_status AS "translationStatus" FROM (
        ...
        UNION ALL SELECT 'location',l.slug,lt.name,lt.summary,w.slug FROM location_translations lt JOIN locations l ON l.id=lt.location_id JOIN works w ON w.id=l.work_id WHERE lt.locale=$1 AND lt.status='published' AND (lt.name||' '||lt.summary||' '||array_to_string(lt.aliases,' ')) ILIKE '%'||$2||'%') s LIMIT 200`, [requestedLocale, parsed.q]);
