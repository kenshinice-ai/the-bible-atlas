...
/**
 * Scripture epigraphs for the Bible Atlas.
 *
 * Editions (revised 2026-08-18):
 * - Chinese: Union Version, Shen (「神」) edition, in the modern-punctuation
 *   simplified setting (新标点和合本). Labelled as such in the UI, because the
 *   quote cards elsewhere in the app carry the 1919 traditional setting of the
 *   same translation and the two must not both be cited as plain 「和合本」.
 * - English: World English Bible. The King James Version was removed here: it
 *   is under perpetual Crown copyright in the United Kingdom, so a globally
 *   served site cannot describe it as public domain, which is exactly what the
 *   footer used to do.
 *
 * "(节选)" / "(excerpt)" marks a shortened quotation. English text is verbatim
 * WEB, retrieved and checked verse by verse during the 2026-08-18 revision.
 */
...
const BIBLE_LOADING: readonly Epigraph[] = [
  ...
  },
  {
    zh: "你的言语一解开就发出亮光。(节选)",
    zhRef: "诗篇 119:130",
    en: "The entrance of your words gives light. (excerpt)",
    enRef: "Psalm 119:130",
  },
  {
