...
import { parseAtlasState, type SelectionMode, type Tab } from "./state";
import { type Atlas,type Locale,type WorksResponse } from "./types";

const MAX_WORKS=3;
const copy={
  "zh-CN":{title:"世界文学名著时空地图",loading:"载入中",characters:"人物",events:"事件",locations:"地点",routes:"路线",sources:"来源",copy:"复制深链接",copied:"已复制",error:"加载失败",all:"叙事进度",single:"单选",multi:"对照多选",choose:"选择世界名著",active:"当前浏览",limit:"最多选择 3 部作品",layer:"现实作品与虚构作品不能叠加在同一地图层",multiHint:"地图叠加已选作品；人物、事件和时间轴跟随当前作品。"},
  en:{title:"World Literature Atlas",loading:"Loading",characters:"Characters",events:"Events",locations:"Places",routes:"Routes",sources:"Sources",copy:"Copy deep link",copied:"Copied",error:"Load failed",all:"Narrative progress",single:"Single",multi:"Compare",choose:"Choose classics",active:"Active work",limit:"Select up to 3 works",layer:"Real and fictional works cannot share one map layer",multiHint:"The map overlays selected works; people, events and timeline follow the active work."}
} as const;
...
export default function App(){
  ...
  useEffect(()=>{let current=true;setError(null);void Promise.all(selectedWorks.map((slug)=>getAtlas(slug,locale))).then((items)=>{if(current)setAtlases(items)}).catch((cause:unknown)=>{if(current)setError(cause instanceof Error?cause.message:String(cause))});return()=>{current=false}},[selectedWorks,locale]);
  ...
  return <main>
    ...
    {error?<section className="error"><strong>{t.error}</strong><p>{error}</p></section>:!activeAtlas?<p>{t.loading}…</p>:<>
      ...
      <aside className="browser"><nav>{(["characters","events","locations","routes"] as Tab[]).map((key)=><button key={key} className={tab===key?"active":""} onClick={()=>setTab(key)}>{t[key]}</button>)}</nav><div className="cards">
        {tab==="characters"&&activeAtlas.characters.map((item)=><article key={item.slug}><h3>{item.name}</h3><p>{item.summary}</p></article>)}
        {tab==="events"&&visibleEvents.map((item)=><article key={item.slug}><span className="sequence">{item.sequence}</span><h3>{item.title}</h3><p>{item.summary}</p><small>{item.startDate??item.reality.replaceAll("_"," ")}</small></article>)}
        ...
        {tab==="routes"&&activeAtlas.routes.map((item)=><article key={item.slug}><h3>{item.name}</h3><p>{item.summary}</p><small>{item.certainty.replaceAll("_"," ")}</small></article>)}
      </div></aside></section>
