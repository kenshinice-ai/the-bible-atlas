...
**2026-07-27 追加(第二轮):画布互动补全。**

- **点击全失效(我上一轮引入的)**:`onPointerDown` 里调 `setPointerCapture`,会把随后的 `click` 重定向到 SVG,子元素的 `onClick` 再也收不到——**整块画布点不动**。改为**只有指针移动超过 4px、确认是拖拽时才捕获**;并加 `suppressClick`,拖完那一下不误选。
  - **验证教训**:上一轮我用 `dispatchEvent(new MouseEvent('click'))` 直接打在 `<g>` 上,绕过了真实指针序列,所以测试通过而实际全坏。**画布交互必须用真实点击验证。**
- **星图与事件/人物从未连通**:`FictionalCanvas` 只认 `selectedEntity.type === "location"`,而真实地图一直通过 `locationForEntity()` 把事件、人物、路线、关系都换算成地点再高亮。虚构画布补上同一步——现在选中「魁刚遇见阿纳金」会点亮塔图因并居中,选中阿纳金同样(出生地),选中科雷利亚大道会点亮整条航线加首个航点。这是「星图跟旁边的列表毫无关系」的根因。
- 路线新增选中态(加粗 + 描边光晕)。

**2026-07-27 追加(第一轮):③⑤ 也已完成,并修掉一个选中态渲染缺陷。**
